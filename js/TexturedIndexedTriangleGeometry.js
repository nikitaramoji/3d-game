"use strict";
const TexturedIndexedTriangleGeometry = function(gl, jsonMesh) {
  this.gl = gl;
  ...
  gl.bufferData(gl.ARRAY_BUFFER,
    new Float32Array(jsonMesh.vertices),
    gl.STATIC_DRAW);

  const indices = [].concat.apply([], jsonMesh.faces);
  ...
  gl.bufferData(gl.ARRAY_BUFFER, 
    new Float32Array(jsonMesh.normals), 
    gl.STATIC_DRAW);

  this.vertexTexCoordBuffer = gl.createBuffer(); 
  gl.bindBuffer(gl.ARRAY_BUFFER, this.vertexTexCoordBuffer); 
  gl.bufferData(gl.ARRAY_BUFFER, 
    new Float32Array(jsonMesh.texturecoords[0]), 
    gl.STATIC_DRAW);
...
TexturedIndexedTriangleGeometry.prototype.draw = function() {
  const gl = this.gl;
